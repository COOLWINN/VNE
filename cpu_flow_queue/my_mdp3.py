...
class MyEnv(gym.Env):
    ...
    def step(self, action):
        ...
        cpu_remain, flow_remain, queue_remain, bw_all_remain = [], [], [], []
        for u in range(self.action_num):
            ...
            if u == action:
                ...
                self.sub.nodes[action]['queue_remain'] -= self.vnr.nodes[self.count]['queue']
            ...
            queue_remain.append(self.sub.nodes[u]['queue_remain'])
        ...
        queue_remain = (queue_remain - np.min(queue_remain)) / (np.max(queue_remain) - np.min(queue_remain))
        bw_all_remain = (bw_all_remain - np.min(bw_all_remain)) / (np.max(bw_all_remain) - np.min(bw_all_remain))
        self.state.current_state(cpu_remain, flow_remain, queue_remain, bw_all_remain)
        reward = (self.sub.nodes[action]['cpu_remain']+self.sub.nodes[action]['flow_remain']+self.sub.nodes[action]['queue_remain']) / (self.sub.nodes[action]['cpu']+self.sub.nodes[action]['flow']+self.sub.nodes[action]['queue'])
