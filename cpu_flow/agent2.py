...
class Agent2:
    ...
    def train(self, sub, req):
        ...
        for i_episode in range(self.episodes):
            ...
            for count in range(req.number_of_nodes()):
                action = self.choose_action(self.p_network, observation, sub, req.nodes[count]['cpu'], req.nodes[count]['flow'])
                if action == -1:
                    break
                else:
    ...
    def choose_best_action(self, model, observation, sub, current_node_cpu, current_node_flow, chosen_nodes):
        ...
        for index, score in enumerate(filter_prob):
            if sub.nodes[index]['cpu_remain'] < current_node_cpu or sub.nodes[index]['flow_remain'] < current_node_flow:
                filter_prob[index] = 0.0
